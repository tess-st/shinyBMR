tabpanel.iml = dashboardPage(
  ...
  dashboardSidebar(sidebarMenu(
    ...
    menuItem("Tree Surrogate", tabName = "surrogate", icon = icon("dice-six")),
    
    prettySwitch(inputId = 'imlInfo', "Show Information", value = FALSE),
    
    hr(),
    
    tags$div(title= "Should computation be parallelized",
      align = "center", uiOutput("iml.parallel.ui")),
    hr(),
    div(align = "center", textOutput("seed.text")),
    hr(),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "surrogate",
        ...
        tabsetPanel(
          tabPanel("Plot: Surrogate Tree",
            fluidRow(fillPage(withSpinner(plotOutput("surrogate_plot")))),
            conditionalPanel(condition = "input.imlInfo == true",
